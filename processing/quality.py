"""Shared data-quality and per-message processing helpers.

Used by both the streaming consumer (pyspark_consumer.py) and the
batch consumer (pyspark_batch.py) so cleaning logic lives in one place.
"""
...
def process_weather_message(data, batch_id):
    import numpy as np
    from datetime import datetime

    # save raw to MongoDB first
    save_raw_to_mongodb('raw_weather', dict(data), batch_id)
...
def process_air_quality_message(data, batch_id):
    import numpy as np
    from datetime import datetime

    # save raw to MongoDB
    save_raw_to_mongodb('raw_air_quality', dict(data), batch_id)
...
def process_carbon_intensity_message(data, batch_id):
    from datetime import datetime
    ...
    if date_exists(cursor, "carbon_intensity_data",
                  record_date):
        cursor.close()
        conn.close()
        print(f"Carbon batch {batch_id}: "
              f"{record_date} already exists, skipped")
        return
